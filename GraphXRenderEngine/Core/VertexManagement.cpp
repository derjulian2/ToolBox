...
grx::VertexArray::VertexArray(VertexArray&& other) noexcept
{
	if (vertex_array_handle)
	{
		release();
	}
	vertex_array_handle = other.vertex_array_handle;
	other.vertex_array_handle = 0;
}
const grx::VertexArray& grx::VertexArray::operator=(VertexArray&& other) noexcept
{
	if (vertex_array_handle)
	{
		release();
...
grx::IndexBuffer::IndexBuffer(IndexBuffer&& other) noexcept
{
	if (index_buffer_handle)
	{
		release();
	}
	index_buffer_handle = other.index_buffer_handle;
	other.index_buffer_handle = 0;
}
const grx::IndexBuffer& grx::IndexBuffer::operator=(IndexBuffer&& other) noexcept
{
	if (index_buffer_handle)
	{
		release();
