...
void grx::Window::draw(Drawable& drawable, const ShaderProgram& program, Camera& camera) const
{
	...
	model_matrix = glm::translate(model_matrix, drawable.getPos());
	model_matrix = glm::rotate(model_matrix, glm::radians(drawable.getRotation().x), glm::vec3(-1.0f, 0.0f, 0.0f));
	model_matrix = glm::rotate(model_matrix, glm::radians(drawable.getRotation().y), glm::vec3(0.0f, -1.0f, 0.0f));
	model_matrix = glm::rotate(model_matrix, glm::radians(drawable.getRotation().z), glm::vec3(0.0f, 0.0f, -1.0f));
	model_matrix = glm::scale(model_matrix, drawable.getScale());
